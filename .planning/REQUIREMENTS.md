# Requirements: Metropolis City Features

**Defined:** 2025-01-23
**Core Value:** The city feels alive and worth watching

## v1 Requirements

### Day/Night Cycle

- [x] **DAY-01**: Sky color transitions smoothly from dawn to day to dusk to night
- [x] **DAY-02**: Sun/moon positioned in sky based on time of day
- [x] **DAY-03**: Ambient and directional lighting changes with time of day
- [x] **DAY-04**: Streetlights turn on at dusk and off at dawn
- [x] **DAY-05**: Vehicle headlights activate during night hours
- [x] **DAY-06**: Building window lights increase at night, decrease during day

### Bird Flocks

- [x] **BIRD-01**: Flocks of birds fly through the city with organic movement
- [x] **BIRD-02**: Birds land on rooftops and sidewalks periodically
- [x] **BIRD-03**: Landed birds scatter when pedestrians approach
- [x] **BIRD-04**: Multiple flocks with varying sizes and flight patterns

### Street Performers

- [x] **PERF-01**: Street performers appear at random sidewalk locations
- [x] **PERF-02**: Nearby pedestrians are attracted to performers
- [x] **PERF-03**: Crowds form in semicircle around performers
- [x] **PERF-04**: Pedestrians watch for a duration then continue walking
- [x] **PERF-05**: Performer types include musician and statue performer

### Subway System

- [ ] **SUB-01**: Subway station entrances visible at street level
- [ ] **SUB-02**: Trains emerge from underground at stations
- [ ] **SUB-03**: Trains stop at platforms for passenger loading
- [ ] **SUB-04**: Pedestrians enter/exit stations
- [ ] **SUB-05**: Trains travel between stations on visible elevated/ground sections

### Rooftop Activity

- [ ] **ROOF-01**: Some buildings have visible rooftop gardens
- [ ] **ROOF-02**: Some buildings have rooftop swimming pools
- [ ] **ROOF-03**: Occasional rooftop parties with string lights
- [ ] **ROOF-04**: Small figures visible on active rooftops

## v2 Requirements

### Enhanced Day/Night

- **DAY-07**: Stars and constellations visible at night
- **DAY-08**: Weather affects lighting (cloudy days darker)

### Enhanced Birds

- **BIRD-05**: Birds react to weather (shelter during rain)
- **BIRD-06**: Different bird species with distinct appearances

### Enhanced Subway

- **SUB-06**: Multiple subway lines with different colored trains
- **SUB-07**: Rush hour crowds at stations

## Out of Scope

| Feature | Reason |
|---------|--------|
| Underground subway view | Complexity, camera is above ground |
| Bird sounds | Audio out of scope |
| Performer music | Audio out of scope |
| Rooftop access | Observation only, no interaction |

## Traceability

| Requirement | Phase | Status |
|-------------|-------|--------|
| DAY-01 | Phase 1 | Complete |
| DAY-02 | Phase 1 | Complete |
| DAY-03 | Phase 1 | Complete |
| DAY-04 | Phase 1 | Complete |
| DAY-05 | Phase 1 | Complete |
| DAY-06 | Phase 1 | Complete |
| BIRD-01 | Phase 2 | Complete |
| BIRD-02 | Phase 2 | Complete |
| BIRD-03 | Phase 2 | Complete |
| BIRD-04 | Phase 2 | Complete |
| PERF-01 | Phase 3 | Complete |
| PERF-02 | Phase 3 | Complete |
| PERF-03 | Phase 3 | Complete |
| PERF-04 | Phase 3 | Complete |
| PERF-05 | Phase 3 | Complete |
| SUB-01 | Phase 4 | Pending |
| SUB-02 | Phase 4 | Pending |
| SUB-03 | Phase 4 | Pending |
| SUB-04 | Phase 4 | Pending |
| SUB-05 | Phase 4 | Pending |
| ROOF-01 | Phase 5 | Pending |
| ROOF-02 | Phase 5 | Pending |
| ROOF-03 | Phase 5 | Pending |
| ROOF-04 | Phase 5 | Pending |

**Coverage:**
- v1 requirements: 24 total
- Mapped to phases: 24
- Unmapped: 0 ✓

---
*Requirements defined: 2025-01-23*
*Last updated: 2025-01-23 after initial definition*
